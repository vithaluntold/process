...
- **Multi-LLM Configuration System**: Production-ready UI-based configuration for multiple AI providers via API Integrations page. Supports 6 OpenAI-compatible providers: Replit AI (free, no setup), OpenAI, Mistral AI, DeepSeek, Groq, and Together AI. Features AES-256-GCM encrypted API key storage, user-scoped provider selection in Settings, one-time key display with masked viewing, and dynamic provider switching. All AI features (Assistant, Anomaly Detection, Predictive Analytics) automatically use the user's selected provider. Database tables: `llm_provider_keys` (encrypted keys), `user_llm_settings` (provider preferences). API endpoints: `/api/llm-providers` (CRUD), `/api/settings/llm` (preference management). NOTE: Azure OpenAI, Anthropic Claude, and Google AI Gemini require custom authentication implementations not yet supported.
